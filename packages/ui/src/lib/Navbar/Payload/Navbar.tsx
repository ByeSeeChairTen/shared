...
import "./style.css";
import {
  DefaultMobileSubItem,
  DefaultSubItem,
} from "./components/defaultSubItem";
import { LinkMobileSubItem, LinkSubItem } from "./components/LinksSubItem";
...
export interface DefaultSubnavbarItem {
  title: string;
  subtitle: string;
  href: string;
}

export interface LinksSubnavbarItem {
  title: string;
  links: Link[];
}

export type SubnavbarItem = DefaultSubnavbarItem | LinksSubnavbarItem;
...
interface MobileNavState {
  open: boolean;
  subnavIndexOpen: number;
}

export const Navbar = (props: NavbarProps) => {
  const [mobileNavState, setMobileNavState] = useState<MobileNavState>({
    open: false,
    subnavIndexOpen: -1,
  });
  ...
  const addRef = (el: HTMLLIElement, i: number) => {
  ...
  const handleMouseEnter = (index: number) => {
    ...
    if (navItem && slidingBar) {
      slidingBar.style.left = `${navItem.offsetLeft}px`;
      slidingBar.style.width = `${navItem.offsetWidth}px`;
  ...
  return (
    <nav className="w-screen flex justify-center bg-black h-20 relative">
      <div className="container max-w-7xl max-sm:px-2 flex max-sm:justify-between sm:grid sm:grid-cols-[repeat(16,1fr)] items-center h-full border-b border-white/20">
        {/* Logo */}
        <div className="text-white max-sm:pl-2 h-full flex items-center sm:col-span-4">
          <svg
            id="logo-81"
            width="72"
            height="40"
        ...
        {/* Navbar Items */}
        <ul
          className="flex gap-x-8 h-full max-sm:hidden sm:col-span-8"
          onMouseLeave={handleMouseLeave}
        >
          {props.navItems.map((item, i) => (
            <li
              key={item.label}
              ref={(el) => el && addRef(el, i)}
              onMouseEnter={() => handleMouseEnter(i)}
              className="group flex items-center cursor-pointer h-full"
            >
              <a
                className="text-white group-hover:text-gray-300 relative"
                href={item.href}
              >
                {item.label}
              </a>
              ...
              {item.subNavbar && (
                <SubNavbar title={item.subNavbar.title} items={item.subNavbar.items} />
              )}
            </li>
          ))}
          <div
            ref={slidingBarRef}
            className="absolute bottom-0 h-0.5 bg-white/80 transition-all pointer-events-none"
          ></div>
        </ul>

        {/* Right Side */}
        <div className="text-white sm:col-span-4 flex">
          {props.rightItems}
          <button
            className="text-white py-1.5 px-2 rounded-xl ml-2 sm:hidden"
            onClick={() =>
              setMobileNavState({
                ...mobileNavState,
          ...
          >
            ...
            <svg
              ...
              className={`h-6 w-6 ${mobileNavState.open ? "hidden" : "block"}`}
              onClick={() =>
            ...
            <svg
              ...
              strokeLinecap="round"
              strokeLinejoin="round"
              className={`h-5 w-5 ${mobileNavState.open ? "block" : "hidden"}`}
              onClick={() =>
                setMobileNavState((prev) => ({ ...prev, open: false }))
              }
            >
              <path d="M3 3l18 18M3 21L21 3"></path>
            </svg>
          </button>
        </div>
      </div>
      ...
      {/* Mobile Nav */}
      <div
        className={`fixed z-20 sm:hidden top-20 bottom-0 left-0 w-screen h-screen bg-black ${
          mobileNavState.open ? "block" : "hidden"
        }`}
      >
        <div className="py-4 flex flex-col">
          {props.navItems.map((item, i) => (
            <div
              key={item.label}
              className="px-4  [&>*:last-child]:border-b-white/20"
              onClick={() => {
                if (item.subNavbar) {
            ...
            >
              <div
                className={`text-white border h-full w-full flex items-center border-b-transparent p-5 border-white/20 text-lg font-medium`}
              >
                {item.label}
                {item.subNavbar && (
                  <>
        ...
        {mobileNavState.subnavIndexOpen !== -1 && (
          <div
            style={{ animationDelay: "0.15s" }}
            className="fixed top-20 bottom-0 left-0 w-screen h-screen animate-fadeIn opacity-0 bg-black"
          >
            <div className="p-4 flex flex-col [&>*:last-child]:border-b-white/20">
              <div
                className="text-white border h-full w-full flex items-center border-b-transparent px-5 py-2.5 pl-2.5 border-white/20 text-base font-regular"
                onClick={() =>
                  setMobileNavState((prev) => ({
                    ...prev,
                    subnavIndexOpen: -1,
              ...
              {props.navItems[
                mobileNavState.subnavIndexOpen
              ]?.subNavbar?.items.map((subItem, i) => {
                if ("subtitle" in subItem && "href" in subItem) {
                  return <DefaultMobileSubItem key={i} subItem={subItem} />;
                } else if ("links" in subItem) {
                  return <LinkMobileSubItem key={i} subItem={subItem} />;
                } else {
                  return null;
                }
              })}
